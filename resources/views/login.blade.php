...
<body class="bg-[#2a2a2a] min-h-screen flex items-center justify-center p-4">
    ...
    <div id="auth-box" class="bg-[#ffedd9] w-full max-w-4xl h-[650px] md:h-[600px] rounded-[2rem] shadow-2xl relative overflow-hidden flex flex-col md:flex-row">
        
        <input type="checkbox" id="toggle-form" class="hidden peer" {{ $errors->any() ? 'checked' : '' }}>

        <div class="hidden md:block absolute top-0 left-0 w-1/2 h-full z-40 transition-transform duration-700 ease-in-out peer-checked:translate-x-full rounded-[2rem] overflow-hidden shadow-2xl">
        ...
        <div class="w-full md:w-1/2 p-8 md:p-12 flex flex-col justify-center bg-[#ffedd9] form-transition
            ...
            <form method="POST" action="{{ route('login.submit') }}" class="flex flex-col gap-4">
                @csrf
                @if(session('error'))
                    <div class="bg-red-100 text-red-700 p-3 rounded-xl text-xs">{{ session('error') }}</div>
                @endif

                <div class="space-y-1">
                    <label class="text-xs font-semibold ml-1">Nama</label>
                    <input type="text" id="log-name" name="name" required class="w-full bg-[#b5a7a4]/20 rounded-xl px-4 py-3 text-sm outline-none focus:ring-2 ring-[#b5a7a4] transition-all">
                </div>
                <div class="space-y-1">
                    <label class="text-xs font-semibold ml-1">Email</label>
                    <input type="email" id="log-email" name="email" required class="w-full bg-[#b5a7a4]/20 rounded-xl px-4 py-3 text-sm outline-none focus:ring-2 ring-[#b5a7a4] transition-all">
                </div>
                <div class="space-y-1">
                    <label class="text-xs font-semibold ml-1">Password</label>
                    <input type="password" name="password" required class="w-full bg-[#b5a7a4]/20 rounded-xl px-4 py-3 text-sm outline-none focus:ring-2 ring-[#b5a7a4] transition-all">
                </div>

                <button type="submit" class="w-full bg-[#b5a7a4] hover:bg-[#a39592] py-4 rounded-xl font-bold text-lg shadow-md mt-4 transition-all active:scale-95">
                    LOGIN
                </button>
            </form>
        ...
        <div class="w-full md:w-1/2 p-8 md:p-12 flex flex-col justify-center bg-[#ffedd9] form-transition
            ...
            <form method="POST" action="{{ route('register.submit') }}" class="flex flex-col gap-3">
                ...
                <div class="space-y-1">
                    <label class="text-xs font-semibold ml-1">Nama</label>
                    <input type="text" id="reg-name" name="name" required class="w-full bg-[#b5a7a4]/20 rounded-xl px-4 py-2 text-sm outline-none">
                </div>
                <div class="space-y-1">
                    <label class="text-xs font-semibold ml-1">Email</label>
                    <input type="email" id="reg-email" name="email" required class="w-full bg-[#b5a7a4]/20 rounded-xl px-4 py-2 text-sm outline-none">
                </div>
                <div class="space-y-1">
                    <label class="text-xs font-semibold ml-1">Password</label>
                    <input type="password" name="password" required class="w-full bg-[#b5a7a4]/20 rounded-xl px-4 py-2 text-sm outline-none">
                </div>
                <div class="space-y-1">
                    <label class="text-xs font-semibold ml-1">Konfirmasi Password</label>
                    <input type="password" name="password_confirmation" required class="w-full bg-[#b5a7a4]/20 rounded-xl px-4 py-2 text-sm outline-none">
                </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const inputs = [
                { log: 'log-name', reg: 'reg-name', key: 'auth_name' },
                { log: 'log-email', reg: 'reg-email', key: 'auth_email' }
            ];

            inputs.forEach(item => {
                const logEl = document.getElementById(item.log);
                const regEl = document.getElementById(item.reg);
                
                const saved = localStorage.getItem(item.key) || '';
                if(logEl) logEl.value = saved;
                if(regEl) regEl.value = saved;

                const syncData = (e) => {
                    const val = e.target.value;
                    localStorage.setItem(item.key, val);
                    if(logEl) logEl.value = val;
                    if(regEl) regEl.value = val;
                };

                if(logEl) logEl.addEventListener('input', syncData);
                if(regEl) regEl.addEventListener('input', syncData);
            });
        });
    </script>
</body>
