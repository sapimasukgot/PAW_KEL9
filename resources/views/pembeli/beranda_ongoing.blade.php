    ...
    <div class="p-6 flex flex-col items-start space-y-8 w-full">
        <div class="w-full">
            <div class="relative w-full">
                ...
                <input type="text" id="search-all" data-translate="holder_search_all"
        ...
        <div class="w-full space-y-4">
            ...
            <div class="space-y-3">
                @forelse($pesanans as $pesanan)
                    <div
                        class="bg-white rounded-3xl p-5 shadow-sm flex items-center border border-orange-100 relative w-full max-w-3xl h-32">
                        <div class="flex items-center gap-5">
                            ...
                            <div class="flex flex-col items-start">
                                ...
                                <p class="text-xs text-gray-400 mt-1">Meja: {{ $pesanan->no_meja }} | Status: <span
                                        class="text-orange-500 font-semibold">{{ $pesanan->status }}</span></p>
                            </div>
                        </div>
                        ...
                        <a href="{{ route('pembeli-detail-pesanan', $pesanan->order_id) }}"
                            class="absolute bottom-5 right-6 text-black font-extrabold text-xs hover:underline">
                            <span data-translate="btn_see_detail">Lihat Detail</span>
                        </a>
                    </div>
                @empty
                    <div
                        class="bg-white/50 rounded-2xl p-6 text-center border border-dashed border-gray-300 w-full max-w-3xl">
        ...
        <div class="w-full space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-3 w-full">
                @forelse($menus as $menu)
                    <div
                        class="card-item bg-white rounded-2xl p-4 shadow-sm flex items-center border border-gray-50 relative h-28 w-full overflow-hidden transition-all duration-200 hover:border-orange-200">
                        <div class="flex items-center gap-3 min-w-0 w-full pr-16">
                            <div
                                class="w-14 h-14 bg-orange-50 rounded-xl flex-none flex items-center justify-center text-xl">
                                🍜
                            </div>
                            <div class="flex flex-col items-start min-w-0 w-full">
                                <h3 class="card-title font-bold text-sm text-gray-900 truncate w-full">
                                    {{ $menu->nama_menu }}</h3>
                                <span class="text-[11px] font-bold text-orange-500 block">Rp
                        ...
                        <a href="{{ route('pembeli-detail', $menu->menu_id) }}"
                            class="absolute bottom-3 right-4 text-blue-600 font-black text-[11px] uppercase tracking-wider hover:text-blue-800 italic"
                            data-translate="btn_order_now">
                            PESAN
                        </a>
                    </div>
                @empty
